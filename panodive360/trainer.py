...
from panodive360.config import REPO_ROOT, num_classes


def calculate_iou(pred, mask, n_classes):
    ious = []
    pred = pred.view(-1)
    mask = mask.view(-1)
...
def train_model(model, train_loader, valid_loader, criterion, optimizer, num_epochs, patience, model_name):
    ...
    runs_root = os.environ.get('PANODIVE360_RUNS', str(REPO_ROOT / 'runs'))
    log_dir = os.path.join(runs_root, model_name)
    ...
    for epoch in range(num_epochs):
        ...
        print(
            ...
            f'- Train accuracy: {train_accuracy:.4f} - Train IoU: {train_iou.mean():.4f}'
        )
        print(
            f'Epoch {epoch + 1}/{num_epochs} - Valid loss: {valid_loss / len(valid_loader):.4f} '
            f'- Valid accuracy: {valid_accuracy:.4f} - Valid IoU: {valid_iou.mean():.4f}'
        )
